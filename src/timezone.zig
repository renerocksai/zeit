...
pub const TZInfo = struct {
    ...
    pub fn parse(allocator: std.mem.Allocator, reader: anytype) !TZInfo {
        var legacy_header = try reader.takeStruct(Header, builtin.target.cpu.arch.endian());
        ...
        if (legacy_header.version == 0) {
            ...
            var header = try reader.takeStruct(Header, builtin.target.cpu.arch.endian());
    ...
    fn parseBlock(allocator: std.mem.Allocator, reader: anytype, header: Header, legacy: bool) !TZInfo {
        ...
        if (!legacy) {
            ...
            const footer_mem = reader.takeDelimiterExclusive('\n') catch |err| switch (err) {
                error.StreamTooLong => return error.OverlargeFooter, // Read more than the buffer holds, much larger than any reasonable POSIX TZ string
                else => return err,
            };
            if (footer_mem.len != 0) {
